...
def _env(key, default=""):
    """Get env var, stripping accidental whitespace/tabs and surrounding quotes.

    This removes accidental single or double quotes stored in .env values
    (e.g. "'8080'" or '"value"') so numeric/env parsing doesn't fail.
    """
...
_provider_chain = [
    ...
    _provider("https://api.openrouter.ai/v1",          "OPENROUTER_API_KEY", "OPENROUTER_MODEL", "openai/gpt-4o-mini"),
    # Ollama — local
...
- web_build: Create a website by generating a complete HTML document with inline CSS and JS. Use when user says "build me a website", "create a portfolio", "make a landing page", etc. Generate beautiful, modern HTML with gradients, animations, and stunning design. After creating, user gets a live preview link at /builds/<name>/. If image URLs are provided in context (uploaded photos or generated images), use them directly in <img src='...'>. CRITICAL: NEVER provide Python code or file editing instructions - ALWAYS call the web_build tool directly with the HTML.
- web_edit: Edit an existing website — provide the full updated HTML to replace the current file. Preserve and reuse any provided image URLs when relevant. CRITICAL: NEVER provide Python code or file editing instructions - ALWAYS call the web_edit tool directly with the HTML.
